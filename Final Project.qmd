...
library(dendextend) # For enhanced dendrogram visualization and customization.
library(circlize) #For creating circular visualizations
...
data1$Education <- ifelse(data1$Education %in% c(1, 2, 3), 0, 
                                 ifelse(data1$Education == 4, 1, 
                                        ifelse(data1$Education == 5, 2, 
                                               ifelse(data1$Education == 6, 3, data1$Education))))
...
#corrplot(cormat)
corrplot(cormat1, method="circle",type="upper" ,diag=FALSE, number.cex = 0.5, tl.cex=1.2,  cl.cex = 1)
```
...
#minbucket = 30 is fine
tree1<-rpart(Diabetes_binary ~ ., data=train.df1, method="class",minbucket=30, cp=0.001)
rpart.plot(tree1)
```
...
```{r}
#tree_cv1 = rpart(Diabetes_binary ~ ., data=train.df1, method="class",minbucket=250 ,cp= 0.0001)
#plotcp(tree_cv1)

tree_cv1 = rpart(Diabetes_binary ~ ., data=train.df1, method="class",minbucket=30 ,cp= 0.001)
plotcp(tree_cv1)
```
...
```{r}
#Chose cp at 0.0022
#tree_cv2 = rpart(Diabetes_binary ~  ., data=train.df1, method="class", minbucket=250 ,cp= 0.00069 )
#rpart.plot(tree_cv2,digits=-2, cex = 0.8)

tree_cv2 = rpart(Diabetes_binary ~ ., data=train.df1, method="class", minbucket=30  ,cp= 0.0022 )
rpart.plot(tree_cv2,digits=-2)
```

## Random Forest

For random Forest, the function we use is:

\$ **randomForest** (formula, data, ntree, nodesize, mtry) \$
...
-   **ntree:** Usually, 100 is enough. Lets start with 100.

-   **nodesize**: Lets start with 50.
...
```{r}
x = train.df1[,-1]   # -1 removes Diabetes_binary

y = train.df1$Diabetes_binary  


set.seed(123, sample.kind="Rejection")
tuneRF (x, y, mtryStart = 3, stepFactor = 2, ntreeTry=20, nodesize=50, improve=0.01)
```

```{r}
# Optimal m_try value is 3, not 4
set.seed(123, sample.kind="Rejection")
rf_final1 = randomForest(Diabetes_binary~., data=train.df1, ntree=20, nodesize=50, mtry= 3)
...
# "HighBP" "BMI" "HighChol" "DiffWalk" "PhysHlth" "HeartDiseaseorAttack" "CholCheck"

#  Variables From Random Forest
# "HighBP" "BMI" "HighChol" "DiffWalk"  "PhysHlth" "HeartDiseaseorAttack"

#  Add extra variables we are interested
# "Stroke" "Smoker" 
```

```{r}
cat("\n*Logistic Regression Model by using original dataset*\n")
logreg0 <- glm(Diabetes_binary ~ HighBP + BMI + HighChol + DiffWalk + PhysHlth + HeartDiseaseorAttack + CholCheck + PhysActivity +  Stroke + Smoker,
...
cat("\n*Logistic Regression Model by using training dataset*\n")
logreg1 <- glm(Diabetes_binary ~ HighBP +  BMI + HighChol + DiffWalk + PhysHlth + HeartDiseaseorAttack + CholCheck+ PhysActivity + Stroke,                
...
logreg1a <- glm(Diabetes_binary ~ HighBP +  BMI + HighChol + DiffWalk + PhysHlth + HeartDiseaseorAttack + CholCheck+ PhysActivity + Stroke,                
...
logreg1b <- glm(Diabetes_binary ~  HighBP +  BMI + HighChol + DiffWalk + PhysHlth + HeartDiseaseorAttack + CholCheck +PhysActivity + Stroke,                
...
#roc.pred1 = prediction(df1$PredProbs1, df1$Diabetes_binary)
#
...
#plot(perf,                      # the data
#     main = "ROC Curve for Original set", # the chart's title
...
```{r}
dend_diabetes2 <- as.dendrogram(hc_diabetes2)
dend_colored1 <- color_branches(dend_diabetes2, k = 5)
par(mfrow = c(1, 1))
plot(dend_colored1, main="Dendogram", xlab="Data Points")
```

```{r}
# Customize dendrogram with colors
dend_colored <- as.dendrogram(hc_diabetes2) %>% 
  color_branches(k = 5) %>%        # Color branches for 5 clusters
  color_labels(k = 5)             # Color labels for 5 clusters

# Create circular dendrogram
circlize_dendrogram(dend_colored,
                    labels_track_height = NA,   # Hide labels (optional)
                    dend_track_height = 0.5)   # Set dendrogram height
```
